        ...
        <layout>
            <div class="x-CenterCon">
                <div style="display: flex;flex-direction: column;">
                    ...
                    <el-button style="margin-top: 5px;width: 100%;" icon="el-icon-search" size="medium" type="primary" @click="getClassroom">{{btnStatus ? "查询" : "链接已超时，请重新回复获取链接"}}</el-button>
                </div>
            </div>
        </layout>
...
<script>
    export default {
        ...
        methods: {
            getClassroom: async function() {
                var res = await $app.ajax({
                    url: `${$app.globalData.url}mp/scl/${this.v1}/${this.v2}/${this.v3}`,
                    headers: {
                        Refer: window.location.href
                    },
                })
                if (res.data.MESSAGE !== "Yes") {
                    $app.toast("链接超时，请于公众号重新回复");
                    return false;
                }
                ...
                if (!data[0]) data = [{"jxl": "青岛校区-" + v3 + "号楼",jsList: [{jsmc: "无空教室"}]}];
                data[0].jsList.sort((a, b) => { return a.jsmc > b.jsmc ? 1 : -1; });
                console.log(data[0]);
                this.room = data[0];
                this.show = 1;
            }
